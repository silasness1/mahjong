...
class Player():  #maybe deck and players inherit from tile collections
    ...
    def __init__(self, name) -> None:
        self.hand = []
        self.lockedTiles = []   #for drawing from graveyard or kongs, prevents reorganizing in checking for mahjong
        self.name = name
    ...
    def displayHand(self, simple = True)-> str:  #TODO: FIX SIMPLE = F
        ...
        lockedDisplay = '\n'.join([lockedHoriz, lockedVert, lockedVert, lockedString, lockedVert, lockedVert,lockedHoriz])
        handDisplay = '\n'.join([handHoriz, handVert, handVert, handString, handVert, handVert, handHoriz])
        if simple:
            return "Hand \n" + "\n".join([f"{str(suitSortedIndex[i] + 1):>2} " + str(sortedHand[i]) for i in range(len(sortedHand))]) + "\nLocked \n" + "\n".join([str(i) for i in self.lockedTiles])
        else:
            return f"{str.upper(self.name)}'s HAND:\nLocked: \n" + lockedDisplay + "\nHand:   \n" + handDisplay
