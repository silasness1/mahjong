...
class GameMaster():
    ...
    def __init__(self, customNames = False) -> None:
        ...
        if customNames == True: 
            for i in range(NUMPLAYERS):
                ...
                thisName = input(queryNameString)         #TODO: make sure no duplicate names to spoof chou privledges 
    ...
    def _peakNextClockwisePlayer(self)->tuple[Player, int]: 
        ...
        nextPlayerIndex = (self.playerList.index(self.activePlayer) + 1) % len(self.playerList)
        return self.playerList[nextPlayerIndex], nextPlayerIndex

    def _advanceNextClockwisePlayer(self)->None:  #active player can change elsewhere in the discard claim.
    ...
    def endgame(self, winner:Player)->None:
        ...
        temp = input() # wait for input before clearing
    ...
    def _discardFromPlayer(self)->None:
        ...
        while True:
            ...
            try:
                discardChoice = int(input(f"Enter 1 to {len(self.activePlayer.hand)} of which card to discard: "))
            except ValueError:
                print("Not an int.")
                continue
    ...
    def _checkLegalMove(self, moveType:int, player:Player, indexList:list[int])->tuple[bool, list[int]]:
        ...
        """
        #check legal move
        ofAKind = check_win.getOfAKindIndices(-1, player.hand + [self.graveyard[-1]]) #done so that if ultimately appended indices correct
        matchCount = len(ofAKind)

        if moveType == 5:   #Mahjong
            legal = check_win.checkMahjong(player.getEffectiveHand() + [self.graveyard[-1]])  #TODO: player.getEffectiveHand isn't returninga s expected
            if not legal:
            ...
            chouIndices = check_win.getChouIndices(-1, player.hand + [self.graveyard[-1]])   #TODO: ensure inability to throw something you just needed to pick up (newly locked tiles)
            if not (chouIndices == [None]*3): 
                if self._peakNextClockwisePlayer()[0].name == player.name: 
                    fakes = [i == None for i in chouIndices]  #seems wrong during debug? 
                    if (sum(fakes) < 2) : #two or three options
                        chouPref = int(input("Which chou do you want? 0 for XOO, 1 for OOX, 2 for OXO")) #TODO: validate input; also appears to be broken. It locked tiles that weren't even one of the options. 
                        while not isinstance(chouIndices[chouPref], list):  #prevents illegal options being none
                            chouPref = int(input("Which chou do you want? 0 for XOO, 1 for OOX, 2 for OXO"))
    ...
    def _callForDiscard(self)->tuple[list[int],list[list[int]]]:
        ...
        prefList = []
        indexList = [] #place to store each player's final desired operation for locked tile transfer. 
        for player in self.playerList:
    ...
    def _advanceToNextMove(self, prefList:list[int], indexList:list[list[int]])->None: 
        ...
        if sum(prefList) == len(prefList):  
            ...
            relativePlayerList = [self.playerList[(i - nextIndex) % len(self.playerList)] for i in range(len(self.playerList))]
            relativePrefList = [prefList[(i - nextIndex) % len(self.playerList)] for i in range(len(self.playerList))]          
        
            # see who had the biggest play (who won the preference list)
            realMax = max(prefList)
            relArgMax = relativePrefList.index(max(relativePrefList)) 
            ...
            self.activePlayer.hand.append(self.graveyard.pop(-1))  #TODO: somehow reorder the indices right to actually get ordered meld to locked tiles. 
            print(f"{self.activePlayer.name} got {self.activePlayer.hand[-1]} from graveyard.")
            
            # move the drawn move to the lockedTiles
            toTransfer = indexList[prefWinnerIndex]
            transferSet = set(toTransfer)
            tilesToLock = [e for i, e in  enumerate(self.activePlayer.hand) if i in transferSet]
            assert len(tilesToLock) > 0 #when obtained from graveyard and not mahjong... something has to go to locked pong or chou. 
            self.activePlayer.lockedTiles += tilesToLock
            self.activePlayer.hand = [e for i, e in  enumerate(self.activePlayer.hand) if i not in transferSet]
    ...
    def takeTurn(self)->None:
        ...
        if check_win.checkMahjong(self.activePlayer.getEffectiveHand()): 
            self.endgame(winner = self.activePlayer) #CW player won from deck or prefWinner got mahjong with grave tile